...
{
	OTA_PACK_HEADER_t *header = (OTA_PACK_HEADER_t*)data;
...
static void ota_update_data_start()
{
	//FileOpen();
	//FileOpen2();
...
static OTA_ERROR_NUM_T ota_update_data_end()
{	
    int status = -1;
	DEBUG_LOGI(OTA_TAG, "OTA: Write Flash end: last_addr:%d left_size:%d\n", last_addr,left_size);
	
    if (left_size > 0) {
        if ((last_addr + PAGE_SIZE) <= OTA_UPGRADE_AREA_SIZE) {
			//FileWrite(cache,PAGE_SIZE);
            status = rda5981_write_partition(last_addr, cache, PAGE_SIZE);
            if (status != 0) {
                DEBUG_LOGE(OTA_TAG, "OTA: Write Flash addr: %d failed\n", last_addr);
				return OTA_WIFI_FAIL;
            }			
			DEBUG_LOGI(OTA_TAG, "OTA: Write Flash end: %d\n", last_addr + left_size);
        }
    } else {
        status = 0;
    }
...
static OTA_ERROR_NUM_T ota_update_data(unsigned char *data, size_t size)
{
    int status = -1;
    unsigned int copy;

	//FileWrite2(data, size);

    if (data && (last_addr + PAGE_SIZE) <= OTA_UPGRADE_AREA_SIZE) {
        if ((left_size + size) < PAGE_SIZE) {
			...
			DEBUG_LOGI(OTA_TAG, "OTA: Write Flash: %d\n", last_addr + PAGE_SIZE);
	...
	size_t *_out_firmware_len)
{
	...
	DEBUG_LOGI(OTA_TAG, "ota server info url:%s domain:%s port:%d",_server_url, socket_data->domain, socket_data->port);
    ...
    while (1) 
	{
        ...
        if (buff_len < 0 && (sock_get_errno(socket_data->sock) == EINTR || sock_get_errno(socket_data->sock) == EAGAIN))
		{
		...
		{ /*deal with packet header*/
...
#endif
			DEBUG_LOGI(OTA_TAG, "ota_update_data_from_server firmware total len = %d\r\n", total_len);
			unsigned int upgrade_max_size = (total_len / 4096 + 1) * 4096; // PARTION_SIZE;

			if(rda5981_write_partition_start(OTA_UPGRADE_ADDR, upgrade_max_size) != 0)
				return	OTA_INIT_FAIL;	
			...
			if (*pBody != 0xAE || buff_len < 2)
			{
				DEBUG_LOGE(OTA_TAG, "buff_len=%d,pBody=%s", buff_len, pBody);
				goto ota_data_from_server_error;
			}
			
			if (ota_update_data((unsigned char*)pBody, buff_len) != OTA_SUCCESS) 
			{
                DEBUG_LOGE(OTA_TAG, "Error: ota_update_audio_write firmware failed!");
                goto ota_data_from_server_error;
            }
		...
		{ /*deal with packet body*/
            if (ota_update_data((unsigned char*)socket_data->str_buf, buff_len) != OTA_SUCCESS) 
			{
                DEBUG_LOGE(OTA_TAG, "Error: ota_update_audio_write firmware failed!");
                goto ota_data_from_server_error;
            }
	...
	if (ota_update_data_end() != OTA_SUCCESS) 
	{
		DEBUG_LOGE(OTA_TAG, "Error: ota_update_data_end firmware failed!");
		goto ota_data_from_server_error;
	}

	//if(rda5981_write_partition_end() != 0)
	//	goto ota_data_from_server_error;
...
OTA_ERROR_NUM_T ota_update_from_wifi(const char *_firmware_url)
{
	...
	/*ota run*/
	if (ota_update_data_from_server(_firmware_url, &firmware_len) != OTA_SUCCESS)
	{
