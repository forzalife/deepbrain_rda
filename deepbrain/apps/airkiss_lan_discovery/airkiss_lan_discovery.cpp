...
static int create_udp_socket(void)
{
	...
	{
		DEBUG_LOGI(LOG_TAG, "udp socket create success[%d]", sock);
	}
	
	if (setsockopt(sock, SOL_SOCKET, SO_BROADCAST, &bcast, sizeof(bcast)) < 0) 
	{
        DEBUG_LOGI(LOG_TAG,"setsockopt SO_BROADCAST failed!");
    ...
    saddr.sin_addr.s_addr = IPADDR_ANY;
	...
	struct sockaddr_in *from_addr)
{
	...
	uint16_t lan_buf_len = sizeof(handle->udp_send_buffer);
...
{		
	...
	else
	{
		...
		DEBUG_LOGI(LOG_TAG, "%s create success", APP_NAME_AIRKISS_LAN_DISCOVERY);
	}
...
APP_FRAMEWORK_ERRNO_t airkiss_lan_discovery_create(const int task_priority)
{
	if (g_airkiss_lan_discovery_handle != NULL)
	{
	...
	g_airkiss_lan_discovery_handle->discovery_status = AIRKISS_LAN_DISCOVERY_STATUS_OFFLINE;
