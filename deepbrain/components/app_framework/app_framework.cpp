	...
	APP_OBJECT_t *app)
{
	...
	//delete msg queue
	app_msg = TAILQ_FIRST(&app->msg_queue);
	while (app_msg != NULL)
	{
	...
	const APP_EVENT_MSG_t *msg)
{
	...
	APP_MSG_QUEUE_ENTRY_t *qe = (APP_MSG_QUEUE_ENTRY_t *)APP_API_MALLOC(sizeof(APP_MSG_QUEUE_ENTRY_t));
	if (qe == NULL)
	{
...
APP_FRAMEWORK_ERRNO_t app_msg_dispatch(APP_OBJECT_t *app)
{
	...
	while(app->run_status == APP_OBJECT_STATUS_RUNNING)
	{
		...
		if (app->loop_callback != NULL 
			&& app->loop_timeout > 0
			&& labs(time_now -  APP_API_TIME_OF_DAY()) >= app->loop_timeout)
		{
	...
	const event_callback_fn ev_cb)
{
	...
	if (is_find == 0)
	{
		APP_EVENT_ENTRY_t *ev_new = (APP_EVENT_ENTRY_t *)APP_API_MALLOC(sizeof(APP_EVENT_ENTRY_t));
		if (ev_new == NULL)
		{
	...
	const uint32_t event_data_len)
{
	...
	msg.event = event;
	msg.event_data = (void *)event_data;
