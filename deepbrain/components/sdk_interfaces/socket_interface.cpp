	...
	char *str_params)
{
	...
	//get domain
	char* domain_start = strstr((char*)str_web_url, "http://");
...
int sock_set_blocking(const sock_t sock)
{
    ...
    rc = lwip_fcntl(sock, F_GETFL, 0);
    if (rc >= 0)
	{
        rc = lwip_fcntl(sock, F_SETFL, rc & (~O_NONBLOCK));
    }
	...
	const size_t ms)
{
	...
	do 
	{
		if (labs(get_time_of_day() - start_time) >= ms)
		{
			break;
		}
