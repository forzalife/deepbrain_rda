	...
	DCL_MPUSH_PUSH_MSG_HANDLER_t **handler)
{
	...
	*handler = new_handler;
	...
	if (sock_get_server_info(MPUSH_SEND_MSG_URL, (char*)&http_buffer->domain, (char*)&http_buffer->port, (char*)&http_buffer->params) != 0)
	{
	...
	const char *to_user)
{
	...
	//发包	
	DEBUG_LOGI(TAG_LOG, "sock send packet:%s",handler->send_buffer);
	...
	//解包	
	DEBUG_LOGI(TAG_LOG, "sock recv packet:%s",http_buffer->str_response);
