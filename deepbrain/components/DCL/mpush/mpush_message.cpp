	...
	MPUSH_MSG_CONFIG_T *_config)
{
	...
	*_out_len = 0;
	...
	MPUSH_MSG_HEADER_T *phead = (MPUSH_MSG_HEADER_T *)_out_buf;
...
void mpush_make_heartbeat_msg(
	...
	size_t *_out_len)
{
	*_out_buf = (char)MPUSH_HEARTBEAT_BYTE;
	*_out_len = 1;
...
static void _mpush_decode_decode_playlist(const char* playlist)
{	
	...
	if (strcmp(pJson_command->valuestring, "playList") == 0)
	{
		...
		else
		{
			...
			for (i=0; i<list_size; i++)
			{
				...
				duer::YTMediaManager::instance().set_music_queue(pJson_url->valuestring);
			}
		}
	}
	else
...
int _mpush_decode_push_msg(char *_body, size_t _body_len, uint8_t *_out, size_t _out_len)
{
	...
	if (NULL != pJson) 
	{
		...
		DEBUG_LOGI(PRINT_TAG, "msg type:%d", msg->msg_type);
		...
		if(msg->msg_type == MPUSH_SEND_MSG_TYPE_TEXT)
		{
			snprintf(msg->msg_content, sizeof(msg->msg_content), "%s", pJson_data->valuestring);
		}
		else if(msg->msg_type == MPUSH_SEND_MSG_TYPE_CMD)
		{
			_mpush_decode_decode_playlist(pJson_data->valuestring);
		}
		else
		{
			snprintf(msg->url, sizeof(msg->url), "%s", pJson_data->valuestring);
		}
		//size_t len = 0;
