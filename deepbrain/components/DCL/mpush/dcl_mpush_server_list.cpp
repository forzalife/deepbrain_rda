	...
	DCL_MPUSH_SERVER_HANDLER_t **handler)
{
	...
	*handler = new_handler;
	...
	if (sock_get_server_info(MPUSH_GET_SERVER_URL, (char*)&http_buffer->domain, (char*)&http_buffer->port, (char*)&http_buffer->params) != 0)
	{
	...
	{	
		...
		if (pBody != NULL)
		{
			...
			if (http_buffer->json_body != NULL) 
			{
		        ...
		        cJSON *pJson_content = cJSON_GetObjectItem((cJSON *)http_buffer->json_body, "content");
