        ...
        size_t len)
{
    ...
    p_client->ops.set_blocking(p_client->ops.n_handle, DUER_TRUE);
        ...
        size_t *p_read_len)
{
    ...
    rev_len = *p_read_len;
    p_client->n_is_chunked = DUER_FALSE;
        ...
        if (is_value_overlength) {
            ...
            if (!DUER_STRNCASECMP(key, "Content-Length", max_len)) {
                ...
                if (!DUER_STRNCASECMP(value, "Chunked", max_len - (value - buf))
                    ...
                    p_client->n_is_chunked = DUER_TRUE;
        ...
        duer_bool *is_relative_url)
{
    ...
    if (p_client->p_location[0] == 'h') {
        *is_relative_url = DUER_FALSE;
        return DUER_HTTP_OK;
    }

    *is_relative_url = DUER_TRUE;
...
{
    duer_bool is_need_connect = DUER_FALSE;
    duer_http_result_t ret = DUER_HTTP_OK;
    int rs = 0;
    int len = 0;

    if (!p_client->last_host) {
        is_need_connect = DUER_TRUE;
        ...
        size_t *remain_size_in_buf)
{
    ...
    char  *p_path          = NULL;
    char  *host            = NULL;
    duer_bool   is_relative_url = DUER_FALSE;
    int    path_len_max    = 0;
    int    ret             = 0;
